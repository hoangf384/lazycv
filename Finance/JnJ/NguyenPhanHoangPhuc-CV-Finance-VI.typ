...
Với nền tảng vững chắc về *Khoa học Dữ liệu trong Kinh doanh* từ Đại học Ngân hàng và sự am hiểu về Tài chính, tôi mong muốn ứng tuyển vị trí **Finance Intern** tại Johnson & Johnson. Định hướng của tôi là phát huy tối đa tư duy phân tích dữ liệu, kỹ năng lập trình (Python) và ứng dụng các giải pháp công nghệ/AI (Power Automate, Microsoft Copilot) để tự động hóa các quy trình lập báo cáo và dự báo tài chính. Tôi hướng tới mục tiêu đồng hành cùng phòng Tài chính với vai trò đối tác kinh doanh (Finance Business Partner) để chuyển đổi dữ liệu thành insights có tính hành động cao, hỗ trợ hiệu quả cho các quyết định thương mại.
...
#grid(
  ...
  column-gutter: 0.5em,
  row-gutter: 0.7em,
  [*Lập trình & AI*], [:], [Python (Pandas, NumPy, SciPy), SQL, R, Kỹ thuật AI Prompting],
  [*Dữ liệu & Tài chính*], [:], [MS Excel (Pivot Tables, Advanced Formulas), Phân tích chênh lệch & xu hướng],
  [*Trực quan hóa (BI)*], [:], [Power BI, Tableau, Looker Studio, Báo cáo tiến độ (Dashboards)],
  [*Cơ sở dữ liệu*], [:], [MySQL, BigQuery, SQL Server],
  [*Công cụ khác*], [:], [Power Automate (Học hỏi), Git, Docker, Microsoft Copilot]
)
...
*#link("https://github.com/walkinglabs/learn-harness-engineering")[Chứng chỉ Kỹ thuật AI Agents (Harness Engineering for AI Agents)]*