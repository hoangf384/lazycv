...
Với niềm đam mê lớn dành cho *Phân tích Dữ liệu* và *Tự động hóa*, tôi luôn giữ tinh thần cầu thị và khao khát học hỏi từ thực tế. Mục tiêu của tôi là ứng dụng công nghệ, tư duy phân tích và các công cụ AI để chuẩn hóa số liệu, tối ưu hóa quy trình vận hành thủ công, qua đó mang lại giá trị thiết thực cho doanh nghiệp.
...
// )
  ...
  desc: [
    - Thu thập, dọn dẹp và chuẩn hóa dữ liệu thô (leads) từ nhiều nguồn trước khi phân bổ khoa học cho đội ngũ Sales, đảm bảo tính nhất quán và bảo mật của dữ liệu.
    - Thiết lập hệ thống bảng tổng hợp và tự động hóa báo cáo vận hành hàng ngày trên Google Sheets bằng các hàm liên kết (IMPORTRANGE, QUERY, FILTER...) và Google Apps Script, hỗ trợ ban quản lý giám sát hiệu suất Sales kịp thời.
    - Chủ động ứng dụng AI (ChatGPT, Gemini) tối ưu hóa các thao tác thủ công, viết công thức và thiết kế các biểu mẫu nhập liệu khoa học để hạn chế sai sót.
  ]
)
...
#grid(
  ...
  column-gutter: 0.5em,
  row-gutter: 0.7em,
  [*Công cụ & AI*], [:], [MS Excel (Pivot Table, Advanced Formulas), Google Sheets, Prompt Engineering (ChatGPT, Gemini)],
  [*Lập trình & SQL*], [:], [Python (Pandas, Polars, PySpark), SQL, Google Apps Script, R],
